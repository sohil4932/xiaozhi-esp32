...
bool AfeWakeWord::Initialize(AudioCodec* codec, srmodel_list_t* models_list) {
    ...
    ESP_LOGI(TAG, "Initializing AFE with wake word model: %s",
             wakenet_model_ ? wakenet_model_ : "(none)");
...
void AfeWakeWord::AudioDetectionTask() {
    ...
    while (true) {
        ...
        // Store audio data for wake word voice encoding
        ...
        if (res->wakeup_state == WAKENET_DETECTED) {
            ...
            Stop();
            ESP_LOGI(TAG, "Wake word detected, stopping detection for server conversation");
        }
    }
}
