...
esp_err_t bsp_audio_play(const int16_t* data, int length, TickType_t ticks_to_wait)
{
    ...
    if (s_bits_per_chan != 32) {
        // Audio files are mono 16-bit, need to convert to stereo 32-bit
        int num_samples = length / sizeof(int16_t);
        out_length = num_samples * 2 * sizeof(int32_t);  // Stereo 32-bit
        data_out = malloc(out_length);
        if (data_out == NULL) {
            ESP_LOGE(TAG, "Failed to allocate memory for audio conversion");
            return ESP_ERR_NO_MEM;
        }
        // Convert mono 16-bit to stereo 32-bit
        for (int i = 0; i < num_samples; i++) {
            int32_t sample = ((int32_t)data[i]) << 16;  // Convert to 32-bit MSB format
            data_out[i * 2] = sample;      // Left channel
            data_out[i * 2 + 1] = sample;  // Right channel (duplicate)
        }
    }
...
esp_err_t bsp_board_init(uint32_t sample_rate, int channel_format, int bits_per_chan)
{
    ...
    /* Initialize codec power control for PCB v1.2 - GPIO48 powers ES7210/ES8311 */
    gpio_config_t  pwr_io_conf;
    memset(&pwr_io_conf, 0, sizeof(pwr_io_conf));
    pwr_io_conf.intr_type = GPIO_INTR_DISABLE;
    pwr_io_conf.mode = GPIO_MODE_OUTPUT;
    pwr_io_conf.pin_bit_mask = ((1ULL << GPIO_NUM_48));
    pwr_io_conf.pull_down_en = 0;
    pwr_io_conf.pull_up_en = 0;
    gpio_config(&pwr_io_conf);
    gpio_set_level(GPIO_NUM_48, 1);
    ESP_LOGI(TAG, "EchoEar PCB v1.2: GPIO48 codec power enabled");

    // Wait for codec power to stabilize
    esp_rom_delay_us(100000);  // 100ms delay

    bsp_i2s_init(I2S_NUM_1, 16000, 2, 32);
    // Because record and play use the same i2s.
    bsp_codec_init(16000, 16000, 2, 32);

    /* Initialize PA (speaker amplifier) - GPIO15 for PCB v1.2 */
    gpio_config_t  pa_io_conf;
    memset(&pa_io_conf, 0, sizeof(pa_io_conf));
    pa_io_conf.intr_type = GPIO_INTR_DISABLE;
    pa_io_conf.mode = GPIO_MODE_OUTPUT;
    pa_io_conf.pin_bit_mask = ((1ULL << GPIO_PWR_CTRL));
    pa_io_conf.pull_down_en = 0;
    pa_io_conf.pull_up_en = 0;
    gpio_config(&pa_io_conf);
    gpio_set_level(GPIO_PWR_CTRL, 1);
    ESP_LOGI(TAG, "EchoEar PCB v1.2: GPIO15 PA enabled");
